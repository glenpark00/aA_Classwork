  ...
  execute(<<-SQL)
    ...
    JOIN
      (
        ...
        WHERE NOT
          ord = 1
      ) AS non_lead_castings
    ON
      movies.id = non_lead_castings.movie_id
    WHERE
      non_lead_castings.actor_id = (
        SELECT
          id
        FROM
          actors
...
def films_and_stars_from_sixty_two
  ...
  execute(<<-SQL)
    ...
    ON 
      movies.id = lead_castings.movie_id
    JOIN 
      actors
    ON
      lead_castings.actor_id = actors.id
    WHERE
      yr = 1962
  SQL
end
...
def prolific_actors
  ...
  execute(<<-SQL)
    ...
    ON
      actors.id = lead_castings.actor_id
    GROUP BY 
      actors.name
    HAVING
      COUNT(*) >= 15
